...
inline void recycle_outgoing6 (struct buffer *buf, struct sockaddr_in6 peer6)
{
    /*
     * This should only be used for ZLB's!
     */
...
/*					c->ourrws = c->lac->call_rws;
	...
	/*                                      c->ourrws = c->lac->call_rws;
		...
		while (y)
		{
			...
			memcmp(y->peer6.sin6_addr.s6_addr, t->peer6.sin6_addr.s6_addr, 16) == 0) &&
		...
		l2tp_log (LOG_NOTICE,
			  ...
			  t->ourtid, t->tid, t->refme, t->refhim);
	 ...
	 * don't ask for sequencing */
		...
		{
		    po = add_opt (po, "ipparam");
		    po = add_opt (po,  t->ipv6 ? IPADDY6 (t->peer6.sin6_addr) : IPADDY (t->peer.sin_addr));
		}
		start_pppd (c, po);
		opt_destroy (po);
		l2tp_log (LOG_NOTICE,
		     "Call established with %s, PID: %d, Local: %d, Remote: %d, Serial: %d\n",
		     t->ipv6 ? IPADDY6(t->peer6.sin6_addr) : IPADDY (t->peer.sin_addr), c->pppd, c->ourcid, c->cid,
		     c->serno);
		break;
#ifndef DISABLE_OCRP_OCCN
...
#endif
	    l2tp_log (LOG_ERR, "%s: sending payload ZLB\n", __FUNCTION__);
	    udp_xmit (buf, t);
	    toss (buf);
	}
